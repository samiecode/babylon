...
const ACTIVE_STATUSES: Array<"PENDING" | "READY"> = ["PENDING", "READY"];
	...
	try {
		...
		if (!["request", "execute", "cancel"].includes(action)) {
			return NextResponse.json(
				{
					success: false,
					error: "action must be request, execute, or cancel",
				},
				{status: 400}
			);
		}
		...
		if (action === "request") {
			...
			if (!amountWei || amountWei <= BigInt(0)) {
				return NextResponse.json(
					{
						success: false,
						error: "amountWei must be provided for request action",
					},
					{status: 400}
				);
			}
		...
		if (!pending) {
			return NextResponse.json(
				{
					success: false,
					error: "No active withdrawal request for this wallet",
				},
				{status: 404}
			);
		}
