			...
			<div className="mx-auto flex w-full max-w-6xl flex-col gap-6">
				<header className="flex flex-col gap-4">
					<div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
						<div>
							<h1 className="text-3xl font-bold text-[#0DF69D] sm:text-4xl">
								Savium
							</h1>
							{overview?.user && (
								<p className="mt-1 text-sm text-gray-400">
									Welcome,{" "}
									<span className="text-[#0DF69D]">
										{overview.user.name ||
											overview.user.email}
									</span>
								</p>
					...
					{isConnected && connectedAddress && (
						<div className="rounded-xl border border-[#0DF69D]/30 bg-[#0DF69D]/10 px-4 py-3 text-sm">
							<span className="font-semibold text-[#0DF69D]">
								✓ Connected:
							</span>{" "}
							<code className="rounded bg-[#0DF69D]/20 px-2 py-1 text-xs text-[#0DF69D]">
								{connectedAddress}
							</code>
						</div>
					)}
					...
					<StatCard
						...
						value={
							!isConnected
								...
								? "Loading..."
								: overview?.user
								? `${overview.user.savingPercentBps}%`
								: "0%"
						}
					/>
					<StatCard
...
function Card({
	...
	return (
		<div className="rounded-xl border border-gray-800 bg-gradient-to-br from-[#0a0a0a] to-[#060101] p-5 shadow-lg sm:p-6">
			<div className="mb-5">
				<h3 className="text-lg font-bold text-[#0DF69D] sm:text-xl">
					{title}
				</h3>
						...
						<table className="min-w-full text-left text-sm">
							...
							<tbody className="divide-y divide-gray-800/50">
								{rows.map((row, idx) => (
									<tr
										key={`${row[0]}-${idx}`}
										className="hover:bg-[#0DF69D]/5 transition-colors"
									>
										{row.map((value, cellIdx) => (
											<td
												key={`${value}-${cellIdx}`}
												className="px-5 py-4 text-gray-300 sm:px-6"
											>
												{value}
											</td>
										))}
